...
file at ``~/.config/sp-base/config.yaml`` (or ``SP_BASE_CONFIG`` env override).
"""
...
DEFAULT_CONFIG_DIR = Path.home() / ".config" / "sp-base"
DEFAULT_CONFIG_FILENAME = "config.yaml"
ENV_CONFIG_PATH = "SP_BASE_CONFIG"


def _get_config_path() -> Path:
    """Resolve the configuration file path.

    Checks the ``SP_BASE_CONFIG`` environment variable first,
    then falls back to ``~/.config/sp-base/config.yaml``.
...
class ConfigService:
    ...
    Args:
        config_path: Optional explicit path to the config file.
            If not provided, uses ``SP_BASE_CONFIG`` env var or
            the default ``~/.config/sp-base/config.yaml``.
    """
