"""Tests for the Bluetooth stale-handle release (Bug D).

A stale BlueZ connection left over from a previous unclean shutdown can
stop the relay claiming the RFCOMM channel, so
``BluetoothManager.disconnect_device(mac)`` is asked first as a
best-effort recovery.  It used to live in ``init_services`` behind the
auto-start check; it now sits in ``bluetooth_service`` and runs on every
path into the relay, and the Verification reuses it with its own manager
(issue #129, decision 4).
...
# The ``reportUnknown*`` suppressions are needed because we exchange
# ``MagicMock`` instances for real types and patch low-level asyncio
# internals; pyright can't narrow either without extensive generic
# plumbing that would obscure the test intent.
...
# ---------------------------------------------------------------------------
# Direct tests on release_stale_bluetooth_handle
# ---------------------------------------------------------------------------
...
class TestReleaseStaleBluetoothHandle:
    ...
    async def test_calls_disconnect_then_close_in_order(
        ...
        await release_stale_bluetooth_handle("AA:BB:CC:DD:EE:FF")
